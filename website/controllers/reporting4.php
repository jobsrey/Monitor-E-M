...
class Reporting extends Controller { 
	...
	function settlement_transaction($dtm1="",$dtm2="",$owner_id=0,$terminal_id=0,$product_id=0,$item_id=0){  
 ...
 function summary_transaction($dtm1="",$dtm2="",$owner_id=0,$terminal_id=0,$product_id=0,$item_id=0){  
 ...
 //exit;
		...
		foreach($tData->result_array() as $k =>  $rowTrx){
			...
			$arr_summary[$rowdt][$rowtid][$rowtype]['item'][$n] 	= $rowTrx['item'];
			$arr_summary[$rowdt][$rowtid][$rowtype]['payment'][$n] 	= $rowTrx['payment']; 
	 ...
	 function summary_kiosk_transactionxx($dtm1="",$dtm2="",$owner_id=0,$terminal_id=0,$product_id=0,$item_id=0){  
		...
		$data['ci_func']  	= $ci_func  	= strtolower(__FUNCTION__); 
		$data['callfunc'] 	= $callfunc   	= '/report_data'; 		 //-- form filter data, submit to this function
		$data['reportfunc'] = $reportfunc 	= $ci_func;//'summary_transaction'; //-- form filter data, submit to this function
		...
		//$strDT = "and ( `date` >= '".$data['dt1']." 00:00:00' and `date` <= '".$data['dt2']." 23:59:59' ) ";  
 		//--- card:
 		 $strDT1s1  = " REPLACE(REPLACE(CONCAT(dt,'',tm),'-',''),':','') >= ".str_replace("-","",str_replace(":","",str_replace(" ","",$data['dt1'].$data['tm1'].":00"))) ;
 
 		 //$strDT1s1b = " SUBSTRING_INDEX(REPLACE(tm,':',''),' ',1) >= ".str_replace("-","",str_replace(":","",str_replace(" ","",substr($data['tm1'],0,5).":00"))) ;
		 $strDT1e1  = " REPLACE(REPLACE(CONCAT(dt,'',tm),'-',''),':','') <= ".str_replace("-","",str_replace(":","",str_replace(" ","",$data['dt2'].$data['tm2'].":59"))) ; 
		 //$strDT1e1b = " SUBSTRING_INDEX(REPLACE(tm,':',''),' ',1)  <= ".str_replace("-","",str_replace(":","",str_replace(" ","",substr($data['tm2'],0,5).":59"))) ; 

 		//--- topup: 
 		 $strDT1s2 = " REPLACE(REPLACE(REPLACE(date,' ',''),':',''),'-','') >= ".str_replace("-","",str_replace(":","",str_replace(" ","",$data['dt1'].$data['tm1'].":00"))) ;
		 $strDT1e2 = " REPLACE(REPLACE(REPLACE(date,' ',''),':',''),'-','') <= ".str_replace("-","",str_replace(":","",str_replace(" ","",$data['dt2'].$data['tm2'].":59"))) ; 
	...
	function summary_all_transaction($dtm1="",$dtm2="",$owner_id=0,$terminal_id=0,$product_id=0,$item_id=0){  
		...
		$strDT = "and ( `date` >= '".$dt1." ".$tm1."' and `date` <= '".$dt2." ".$tm2."' ) ";  
		 ...
		 $q = "
			Select id,	kdtrx,	'crd' as kdprd,	vmid,	CONCAT(dt,' ',tm) as date,	totalbayar, uangmasuk,	uangkembali, jmlbeli, nourut as trx_num, null as slot, null as totaltrx
			From `trx` 
			Where $strTRXID ".str_replace("terminal_id","vmid",$strTID)."  $strDT1s1 $strDT1e1  
			
			UNION
			
			Select id,	'TOPUP' as kdtrx,	'tup' as kdprd, terminal_id as vmid, date, value, uangmasuk, uangkeluar, 1 as jmlbeli, trx_counter as trx_num, slot, null as totaltrx
			...
			Select id,	kdtrx,	'tbk' as kdprd,	vmid , CONCAT(dt,' ',tm) as date, null as value, uangmasuk, null as uangkeluar, null as jmlbeli, null as trx_num, null as slot, totaltrx
	...
	function report_data(){ 
		...
		if($_POST){ 
		    ...
		    $str_redir = '/'.strtolower(__CLASS__).'/'.$reportfunc.'/'.$dt1.'--'.str_replace(":",".",$tm1).'/'.$dt2.'--'.str_replace(":",".",$tm2).'/'.$owner_id.'/'.$terminal_id.'/'.$product_id.'/0'; 
