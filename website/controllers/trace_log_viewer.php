...
class Trace_log_viewer extends Controller { 
	...
	function card_transaction_mandiri($curpage=1,$limit=10,$statusid=1,$sort="dt",$order="desc",$ci_ctrl="trace_log_viewer",$ci_changePage="changepage",$result_id="dspdata",$type=1){  
     	$data['pageTITLE']	= 'Lunari Web Admin - Trace Log Viewer'; 
	...
	//topup_mandiri
	function topup_transaction_mandiri($curpage=1,$limit=10,$statusid=1,$sort="date",$order="desc",$ci_ctrl="trace_log_viewer",$ci_changePage="changepage",$result_id="dspdata",$type=4){  
     	$data['pageTITLE']	= 'Lunari Web Admin - Trace Log Viewer'; 
	...
	function topup_mandiri_balance($curpage=1,$limit=10,$statusid=1,$sort="id",$order="desc",$ci_ctrl="trace_log_viewer",$ci_changePage="changepage",$result_id="dspdata",$type=6){  
     	$data['pageTITLE']	= 'Lunari Web Admin - Trace Log Viewer'; 
     	$data['pageLABEL'] 	= 'Log Viewer > History Topup Mandiri Balance';
		$data['navMenu']   	= $this->allfunc->navMenu(6);   
		$data['limit'] 		= $limit;
		$data['statusid'] 	= $statusid;
		$data['sort'] 		= $sort;
	...
	function indexDataLog($curpage,$limit,$status_id,$sort,$order,$ci_ctrl,$ci_changePage,$result_id,$type){ 
		...
		if($start >= 0){
			...
			elseif($type==1){ 
				...
				$logType = "card_transaction_mandiri";
			} 
			elseif($type==2){ 
			...
			elseif($type==4){ 
				...
				$logType = "topup_transaction_mandiri";
			...
			elseif($type==6){ 
				$arr_fields = array('id','terminal_id','TID','date','ka_slot','ka_balance','ka_date','string_request','status_id'); 
				$qData=$this->allmodel->selectLog($start,$limit,$status_id,$sort,$order,'topup_mandiri_balance'); 
				$logType = "topup_mandiri_balance";
				$w = 500;
				$h = 300;
			}
