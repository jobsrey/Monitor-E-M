	...
	<div class="lblpage" align="right">
		...
		<div class="float_r arial12 grey9">
		  Status : <? $liststatus=array(array(1,'Active'),array(2,'Delete')); ?>  
                ...
                onchange="loadcontent('<?=base_url()?>member_manage/changepage/1/<?=$limit?>/'+$('select#selStatus').val()+'/<?=$sort?>/<?=$order?>/<?=$ci_ctrl?>/<?=$ci_changePage?>/<?=$result_id?>','<?=$result_id?>')"> 
        ...
        <select name="GroupID" id="GroupID" onchange="window.location.assign('<?=base_url().$ci_ctrl;?>/members/1/<?=$limit?>/<?=$statusid;?>/<?=$sort?>/<?=$order?>/'+$('select#GroupID').val())" > 
