        ...
        <table cellpadding="3">
          <tr>  
            ...
            <div id="tproductlist" class="float_l">
                ...
                */ ?> 
                <? if($notif_vmselect) echo '<span class="red">*select terminal!</span>'; ?>
            </div>            </td> 
          </tr>
          <tr>
            <td>Date Time
    	...
    	<div id="dspdata"> 
          ...
          <table class="tbldetail2 tdline" width="" border="0" cellpadding="5" cellspacing="0">
			...
			*/
       	  ...
       	  <table class="tbldetail2 tdline" cellpadding="2" cellspacing="1" bgcolor="#CCCCCC">
				...
				<thead bgcolor="#efefef">
					<tr >
                        ...
                        <td rowspan="2" style="border-bottom:2px solid #006699;"><strong>Terminal</strong></td> 
                        <td rowspan="2" style="border-bottom:2px solid #006699;"><strong>Date Time</strong></td>  
					    <td colspan="2" align="center" bgcolor="#F2D0C6"><strong>eMoney Card - Mandiri </strong></td> 
					    <td colspan="4" align="center" bgcolor="#B3DFF2"><strong>eMoney Top-Up - Mandiri</strong></td>
				...
				<tbody>
                    <? 
					$x=1; 
					$tot_quaCrd = $tot_amountCrd = $tot_qua50 = $tot_qua100 = $tot_amountTup = 0;
					foreach($rowTrx as $row){
						...
						$quaCrd = $amountCrd = $qua50 = $qua100 = $amountTup = $txSlot = '';
						...
						if ($row['kdprd'] == 'crd') {
							...
							if(!empty($amountCrd)) 	$tot_amountCrd 	+= $amountCrd; 
							...
							if(!empty($qua50)) 		$tot_qua50 		+= $qua50;  
							if(!empty($qua100)) 	$tot_qua100 	+= $qua100; 
							if(!empty($amountTup)) 	$tot_amountTup 	+= $amountTup; 
			...
			?>
					<tr bgcolor=" <? if($row['kdprd']!='tbk') echo"#ffffff"; else echo "#996633";?>" > 
					  <td align="right"><? if($row['kdprd']!='tbk') echo $x."."; else $x=$x-1; ?>&nbsp;<input name="id" type="hidden" value="1" /></td> 
                        
					    <td <? if($row['kdprd']=='tbk') echo $clr_tbk;?> ><?=$row['vmid'];?></td> 
						<td <? if($row['kdprd']=='tbk') echo $clr_tbk;?> ><?=$row['dt'];?> <?=$row['tm'];?><?=$row['date'];?></td>  
						
						<? if($row['kdprd']=='tbk'){ ?>
                        
                        <td colspan="6" align="right"><span class="" style="color:#F5D996;">Settlement Amount :</span></td>
                        <td align="right" <? if($row['kdprd']=='tbk') echo $clr_tbk. 'align="left"'; else echo 'align="right"';?>><?=number_format($row['totalbayar'],0,',','.');?></td>
						<td align="center" <? if($row['kdprd']=='tbk') echo $clr_tbk;?>><? if($row['kdprd']=='tbk') echo $row['totaltrx'];  else echo $row['jmlbeli'];?>						  <? //if($row['kdprd']=='tbk')  echo $row['uangmasuk']; else echo number_format($tot,0,',','.');?></td>
                        <td align="center"><span class="fgothic size16 white"><? if($row['trx_num']) echo $row['trx_num']; else echo "-";?></span></td>
						...
						<td <? if($row['kdprd']=='tbk') echo $clr_tbk;?> align="right"><?=number_format($row['totalbayar'],0,',','.');?></td>
						<td <? if($row['kdprd']=='tbk') echo $clr_tbk;?> align="center" style="color:#999999"><? if($row['kdprd']=='tbk') echo $row['totaltrx'];  else echo $row['jmlbeli'];?>						  <? //if($row['kdprd']=='tbk')  echo $row['uangmasuk']; else echo number_format($tot,0,',','.');?></td>
