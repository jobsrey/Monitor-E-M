...
class Funcdbindex extends Allmodel{ 
		...
		if($qData->num_rows()){  
			...
			foreach($arrResult as $k => $row){
				...
				$arr_id = $arr_parent = $arr_loc = array();
						...
						if(count($arr_parent)==5) $cur_item_cat = 'kelurahan'; //echo "is_venue<br>"; 
						if(count($arr_parent)==4) $cur_item_cat = 'kecamatan'; //echo "is_area<br>"; 
						if(count($arr_parent)==3) $cur_item_cat = 'kotakab'; //echo "is_city<br>"; 
						if(count($arr_parent)==2) $cur_item_cat = 'provinsi'; // echo "is_province<br>";
						if(count($arr_parent)==1) $cur_item_cat = 'negara'; //echo "is_country<br>";
	...
	function hiddenfields($varKey,$id=0){ 
		if($id==1)
		$arrhide = array(
		'status_id','location_id','owner_id','parent_id',
		'lineage','link','note','status','kecamatan','provinsi','negara','video_id',
	...
	function hiddenfieldsedit($varKey){
		$arrhide = array('online_status','kecamatan','provinsi','provinsi','negara','modified_date','modified_by');
		$res = 0;
		foreach($arrhide as $k => $v) if($v==$varKey)$res = 1;
		return $res; 
	} 
	...
	function indexDataTable($ci_ctrl='',$ci_func='',$TABLE='',$curpage=1,$limit=10,$status_id=1,$sort='id',
		...
		if($qData->num_rows()){ 
			...
			$content.=
				'<table id="tableindex" width="" border="0" cellpadding="7" cellspacing="1" bgcolor="#bbbbbb">
				   <thead>
					  <tr bgcolor="#666666">
					  	...
					  	if(!$this->hiddenfields($varKey,1)){
							...
							elseif($varKey=='suplier_id'){
								...
								if($useTable){
									...
									if($useTable=='md_card_vendor') $useKey = 'vendor';
	...
	function editFormProductVT($data){  //--   edit product vending terminal 
		...
		$str_redir = base_url().$ci_ctrl."/edit_data/".$id."/vending_terminal/".$cur_product_id."/".$cur_card_vendor_id."/".$cur_product_delivery_id."/".$cur_trx_code_id;
			...
			if(!$this->hiddenfields($varKey)){ 
				...
				if( $this->showfieldseditproduct($varKey) ){  // echo "<br>$key ===> $varKey"; 
					...
					if($varKey=="product_id"){ 
						...
						$arrVendor      = explode(",",$card_vendor_id);
						...
						if(count($arrProduct_id)&&!empty($arrProduct_id[0])){					  
							foreach($arrProduct_id as $pkey => $pid){ //echo "<br>$pid"; 
								...
								//$rowProduct 	= $this->allmodel->getTableByID('pd_product',$arrProduct_id[$pkey]);
								$rowVendor 		= $this->allmodel->getTableByID('md_card_vendor',$arrVendor[$pkey]);
								//$rowDelivery 	= $this->allmodel->getTableByID('md_delivery_methods',$arrDelivery[$pkey]);
								...
								*/
								...
								$rowDelivery 	= $this->allfunc->listmenu('md_delivery_methods','delivery_type',1,$arrDelivery[$pkey],'',0,'delivery_type'.$pkey,'','');
							...
							if($add){	
								...
								$rowProduct 	= $this->allfunc->listmenuarray2($lmenu,$datalist,$str_onchange); 
								$rowDelivery 	= $this->allfunc->listmenu('md_delivery_methods','delivery_type',1,0,'',0,'delivery_type'.$add_id,'','');
					...
					elseif($product_id&&$card_vendor_id&&$product_delivery_id){  
		...
		foreach( $arr_fields as $key => $varKey){ //echo "<br>$key => $varKey"; 
			...
			if(!$this->hiddenfields($varKey)){
				...
				if(!$this->hiddenfieldsedit($varKey)){
					...
					elseif($varKey=="card_vendor_id"){ 
						$result .= '  <td>';  
						$result .= $this->allfunc->listmenu('md_card_vendor','vendor',1,$product_id,'',0,'vendor_id','','none Vendor'); 
						$result .= '  </td>';  
					} 
					...
					elseif($varKey=="suplier_id"){  
						...
						$selected1 = $selected2 = '';
						if($useTable=='md_card_vendor') 	 { $useKey = 'vendor';		$selected1 = ' selected="selected" ';} 
						...
						$result .= '	<option value="md_card_vendor" '.$selected1.' >Product Vendor</option>';
	...
	function editFormTable($data=array()){  
		...
		$cur_card_vendor_id   = $card_vendor_id;  
			...
			if(!$this->hiddenfields($varKey)){
				...
				if(!$this->hiddenfieldsedit($varKey)){
					...
					if($varKey=="venue"){ 
							...
							$qDataLoc=$this->allmodel->selectIndexTable(0,1000,1,'venue','asc','md_location');  
							...
							$result .= '<br><div style="padding:5px 0 0;">
										<input type="button" name="button1" id="button1" value="Edit Location" onclick="window.open(\'/master_data/edit_data/\'+$(\'select#location_id\').val()+\'/location/0/1\',\'edit_location\',\'height=1024,width=800\')" />
								   		</div>'; 
							$result .= '  </td>';
					...
					elseif($varKey=="terminal_owner"){ 
							...
							//if($product_id&&$card_vendor_id&&$product_delivery_id)
					...
					elseif($varKey=="video_id"){
							...
							$result .= '  <td>'; 
							//$result .= $this->allfunc->listmenu($table='md_terminal_owner',$rowname='name',$status_id=1,$selected=0,$where_key='',$where_val=0,$listname='owner_id',$result_id='',$firstitem='select owner:');
							$result .= '<div style="padding:2px 0;">Screen Video: '. $this->allfunc->listmenu('media_database','title',1,$video_id,'type','video','video_id','','select video:').' &nbsp; <input style=" font-size:10px;" type="button" name="button1" id="button1" value="Upload New Video" onclick="window.open(\'/master_data/edit_data/'.$arr_fields['location_id'].'/location\',\'edit_location\',\'height=1024,width=800\')" /></div>';
					...
					elseif($varKey=="id" && $data[$varKey]=='Lunari' && $cur_id==1){ 
					//-- As default item, can't change this key & value. 
						$result .= '  <td><strong>'.$data[$varKey].'</strong></td>';
					}				
					elseif($varKey=="MediaId" && $this->session->userdata('GroupID') != '1'){ 
					//-- As default item, can't change this key & value. 
						$result .= '  <td>'.$this->allmodel->getGroupbyID($this->session->userdata('GroupID')).'
						
						<input name="MediaId" type="hidden" id="MediaId" value="'.$this->session->userdata('GroupID').'" size="1" />
						</td>';
					}
					else { 
				...
				if(!$this->hiddenfieldsedit($varKey)){
					...
					if($varKey=="venue"||$varKey=="terminal_owner"||$varKey=="products"){
						...
						if($varKey=="venue"){
							...
							$result .= '<br><div style="padding:5px 0 0;">
										<input type="button" name="button1" id="button1" value="Edit Location" onclick="window.open(\'/master_data/edit_data/'.$arr_fields['location_id'].'/location\',\'edit_location\',\'height=1024,width=800\')" />
								   		</div>
								    </td>'; 
						}
						elseif($varKey=="terminal_owner"){
