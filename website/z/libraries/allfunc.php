...
class Allfunc extends Allmodel{ 
	...
	function navMenu($slcid=-1,$mode=0){
		...
		//--- e:2016 : acceses ID for : member and merchant
		//echo '<br>is_admin : '.$this->session->userdata('is_admin');
		//echo '<br>is_member : '.$this->session->userdata('is_member');
		//echo '<br>is_merchant : '.$this->session->userdata('is_merchant');
			
		$accessid = $this->session->userdata('AccessID');
			
		//if($this->session->userdata('is_member'))   $accessid = "3,26,28,39,29";
		//if($this->session->userdata('is_merchant')) $accessid = "4,30,40,32,31";
		...
		if($qdata->num_rows()){
			...
			if( $this->showmenu($row['ID'],$accessid) ){
