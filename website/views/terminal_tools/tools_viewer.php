...
var ipsrv = "mandiri.lunarikiosk.com";//"<?=server_ip();?>"; 
var clss = "<? echo "update_".$clss;?>"; 
var tip = "<?=$rowtid['IP'];?>";
var tid = "<?=$rowtid['terminal_ID'];?>"; 
var ipvm = tip+'/'+tid;
 
function vmping(msg){    
	//var ipvm = 0;  
	var type = 'vmtools'; 
	url='http://'+ipsrv +'/request/vmping/'+msg;	
	//alert(url);
	openCenteredWindow(url); 
	//openPerfectPopup(oW,oTitle,oContent)
	return false; 
}
...
function slcvm($tid){ 
    if(clss=='update_jumlah_kartu'||clss=='update_generate_passwords'||clss=='update_replenish_approval') var gourl = 'replenishment';
	else var gourl = 'terminal_tools'; 
	//var url = '/terminal_tools/'+clss+'/'+$tid;
	var url = '/'+gourl+'/'+clss+'/'+$tid;
	//alert(url);
	window.location.assign(url); 
    ...
    <div class="linelbl"></div> 
  	<div id="contMenu"> 
       ...
       <table border="0" cellpadding="2" cellspacing="2">
        <tr>
          <td> <input type="button" name="Submit2" onclick="vmping('PING')" value="CHECK ALL TERMINALS"   style="" />  
	  ...
	  <div id="dspdata"> 
        ...
        <tr>
          ...
          <td nowrap="nowrap"><?=$terminal_ID;?></td>
        </tr>
        <tr>
          <td>Title</td>
          <td>:</td>
          <td nowrap="nowrap"><?=$terminal_name;?></td>
        </tr>
        <tr>
          <td><span class="style6">Address</span></td>
          <td><span class="style6">:</span></td>
          <td nowrap="nowrap"><?=$terminal_address;?></td>
        </tr>
        <tr>
          <td><span class="style6">Location</span></td>
          <td><span class="style6">:</span></td>
          <td nowrap="nowrap"><?=$venue;?>, <?=$area;?>, <?=$city;?>, <?=$province;?>, <?=$country;?></td>
        </tr>
        <tr>
          <td><span class="style6">Owner</span></td>
          <td><span class="style6">:</span></td>
          <td nowrap="nowrap"><?=$terminal_owner;?></td>
        </tr>
      </table>
	  <hr size="1" /> 
      ...
      <table border="0" cellpadding="5" cellspacing="1">
        ...
        <tr bgcolor="#CCCCCC">
          ...
          <td align="center"><input disabled  name="button"  type="button"  style="font-size:18;padding:2px 10px;color:#999;" onclick="vmtools('RESTART')" value="RESTART" /></td>
          <td align="center"><input disabled type="button" name="Submit222" onclick="vmtools('SHUTDOWN')" value="SHUTDOWN" style="color:#999;" /></td>
        </tr> 
        <tr>
        ...
        <tr bgcolor="#CCCCCC">
          ...
          <td align="center"><input type="button" name="Submit2232" onClick="vmtools('BA')" value="BILL ACCEPTOR"  disabled style="color:#999"/></td>
          <td bgcolor="#FFFFFF">&nbsp;</td>
	  ...
	  <? } elseif($clss=='video') {  //foreach($qMedia->result_array() as $row) foreach($row as $k => $v) echo "<br>$k => $v"; 
	  ?>
	  ...
	  &nbsp;  &nbsp;
	  <input type="submit" name="Submit" onClick="sendfiletovm($('select#selectvideo').val(),'video')" value="SEND to TERMINAL"> 
	  </div> 
	  

	  <? } elseif($clss=='patch') { ?>
	  
	  <div style="padding:12px 5px;"> Pilih patch file: &nbsp; &nbsp; 
	  <select name="selectpatch" id="selectpatch"> 
        <option value=""  selected="selected" ></option>
         <? $slc_id = 0;
		 	if($qMedia->num_rows()){   
				$x=0;
	  ...
	  <? } elseif($clss=='jumlah_kartu') { ?> 
      
	  <div style="padding:12px 5px;">UPDATE JUMLAH KARTU : &nbsp; &nbsp;   
        <input name="chgcard" type="text" id="chgcard" value="" size="2" /> &nbsp; &nbsp;    
		<input type="button" name="Submit22322" onclick="vmtools('JCARD'+$('input#chgcard').val())" value="SUBMIT" /> 	
	  </div>    
	  
	  <? } elseif($clss=='generate_passwords') { ?>  
	  ...
	  <div style="padding:12px 5px">  
			...
			&nbsp;&nbsp;&nbsp;=&gt; &nbsp;&nbsp;&nbsp;
            <input type="button" name="Submit" onclick="vmtools('passw1='+$('input#passw1').val()+';passw2='+$('input#passw2').val())" value="SUBMIT" />
	  </div>   
